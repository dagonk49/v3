...
/**
 * V0.012 — VideoPlayer powered by hls.js
 * - Multi-audio : changement de piste VF/VO via hls.audioTracks en temps réel
 * - Sous-titres : support VTT externes + ASS/SRT internes (Jellyfin transcodes to VTT)
 * - Interface mobile : contrôles tactiles agrandis, volume slider toujours visible,
 *   zones de double-tap pour skip ±10s
 * - HLS fallback → Safari native → Direct Play
...
export function VideoPlayer({ item, episodeId, onClose }) {
  ...
  // ── Fetch stream metadata (with retry support) ──
  useEffect(() => {
    let cancelled = false;
    (async () => {
      try {
        const id = episodeId || item?.localId || item?.id;
        if (!id) { setError('ID manquant — impossible de lancer la lecture'); setLoading(false); return; }
        ...
        if (r.streamUrl) {
          ...
          setError('Ce média n\'est pas encore disponible localement. Faites une demande via le bouton Demander.');
        } else {
          setError(r.error || 'Stream indisponible — vérifiez que le média est disponible sur le serveur');
        }
      } catch (e) {
        if (!cancelled) {
          console.error('[VideoPlayer] Stream fetch failed:', e.message);
          setError(`Connexion au serveur échouée : ${e.message}`);
        }
      }
      if (!cancelled) setLoading(false);
    })();
    return () => { cancelled = true; };
  }, [episodeId, item?.localId, item?.id, retryCount]);
  ...
  useEffect(() => {
    ...
    if (Hls.isSupported()) {
      ...
      hls.on(Hls.Events.ERROR, (_, data) => {
        if (data.fatal) {
          console.warn('[HLS] fatal error, trying direct play fallback', data.type, data.details);
          hls.destroy();
          hlsRef.current = null;
          if (streamData.fallbackStreamUrl) {
            ...
            setError('Lecture HLS impossible. Essayez de relancer le lecteur.');
          }
        }
      });
  ...
  // ── Double-tap to skip ±10s on mobile ──
  const handleVideoAreaTap = useCallback((e) => {
    ...
    if (dt < 350 && dx < 80) {
      ...
      // Single tap → toggle controls after a small delay
      if (doubleTapTimer.current) clearTimeout(doubleTapTimer.current);
      ...
      {loading ? (
        ...
        <div className="text-center max-w-md px-6">
          ...
          <div className="flex gap-3 justify-center">
            <Button onClick={() => { setError(''); setLoading(true); setStreamData(null); setRetryCount(c => c + 1); }} className="bg-red-600 hover:bg-red-700 rounded-xl">
              Réessayer
            </Button>
            <Button onClick={onClose} className="bg-white/10 hover:bg-white/20 rounded-xl">
              Fermer
            </Button>
          </div>
        </div>
        ...
        <div ref={containerRef} className="w-full h-full relative">
          <video
            ...
            onError={(e) => {
              // Capture les erreurs vidéo natives (codec non supporté, source inaccessible...)
              const mediaErr = e.target?.error;
              if (mediaErr && !hlsRef.current) {
                console.error('[VideoPlayer] Native video error:', mediaErr.code, mediaErr.message);
                if (mediaErr.code === MediaError.MEDIA_ERR_SRC_NOT_SUPPORTED) {
                  setError('Format vidéo non supporté par le navigateur.');
                } else if (mediaErr.code === MediaError.MEDIA_ERR_NETWORK) {
                  setError('Erreur réseau — le serveur vidéo est peut-être inaccessible.');
                } else {
                  setError(`Erreur de lecture (code ${mediaErr.code})`);
                }
              }
            }}
          ...
          <AnimatePresence>
            {doubleTapSide && (
              <motion.div
                ...
                exit={{ opacity: 0 }}
                className={`absolute top-1/2 -translate-y-1/2 ${doubleTapSide === 'left' ? 'left-12' : 'right-12'} w-16 h-16 rounded-full bg-white/20 backdrop-blur flex items-center justify-center pointer-events-none z-20`}
              >
          ...
          <div className={`absolute inset-0 transition-opacity duration-500 z-10 ${showControls ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
            ...
            {/* Bottom bar */}
            <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/90 to-transparent p-4 sm:p-6 pt-16">
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-1 sm:gap-2">
                  {/* Volume — always visible on mobile */}
                  ...
                  {subtitlesMeta.length > 0 && (
                    <div className="relative">
                      <button data-testid="player-subs-toggle" onClick={() => { setShowSubMenu(!showSubMenu); setShowAudioMenu(false); }} className={`p-2 rounded-xl transition-all min-w-[40px] min-h-[40px] flex items-center justify-center ${activeSub >= 0 ? 'bg-white/20 text-white' : 'hover:bg-white/10 text-gray-400'}`}><Subtitles className="w-5 h-5" /></button>
                      {showSubMenu && (
                    ...
                    <div className="relative">
                      ...
                      {showAudioMenu && (
                        <div className="absolute bottom-12 right-0 glass-strong rounded-2xl p-2 min-w-[220px] shadow-2xl">
                          {audioTracksMeta.map((a, i) => <button key={i} onClick={() => handleAudioTrack(a.index ?? i)} className={`w-full text-left px-4 py-3 sm:px-3 sm:py-2 rounded-xl text-sm ${a.isDefault ? 'bg-white/10 text-white' : 'text-gray-400 hover:bg-white/5'}`}>{a.displayTitle}{a.channels ? ` (${a.channels}ch)` : ''}</button>)}
                        </div>
                      )}
                    </div>
                  )}
