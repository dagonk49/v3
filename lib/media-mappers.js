 ...
 */
/**
 * Mappe un item TMDB/Seerr vers le format unifié DagzFlix.
 * Gère les IDs TVDB/TMDB et protège contre les acteurs dans les recherches médias.
 */
export function mapTmdbItem(item, forceTv = null) {
  const mediaType = item.mediaType || item.media_type || '';
  const isPerson = mediaType === 'person';
  const isTv = forceTv ?? (mediaType === 'tv');

  const genreIds = item.genreIds || item.genre_ids || [];
  const posterPath = item.posterPath || item.poster_path || item.profilePath || item.profile_path || '';
  ...
  // Protection Seerr : priorité absolue au vrai tmdbId, car item.id peut être un tvdbId
  const realTmdbId = item.tmdbId || item.id;
  const realTvdbId = item.tvdbId || item.tvdb_id || null;
  ...
  return {
    ...
    mediaType: isPerson ? 'person' : (isTv ? 'tv' : 'movie'),
